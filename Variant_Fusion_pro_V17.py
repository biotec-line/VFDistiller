...
# FASTA Resources
FASTA_URLS = {
    "GRCh37": "https://ftp.ensembl.org/pub/grch37/current/fasta/homo_sapiens/dna/Homo_sapiens.GRCh37.dna.primary_assembly.fa.gz",
    "GRCh38": "https://ftp.ensembl.org/pub/current_fasta/homo_sapiens/dna/Homo_sapiens.GRCh38.dna.primary_assembly.fa.gz",
}
...
class App(ttk.Window):
    ...
    def _attach_tooltip(self, widget, text: str, delay_ms: int = 350):
        """Ergaenzt kompakten GUI-Controls einen fokussierbaren Hilfetext."""
    ...
    def _build_gui(self):
        ...
        input_entry = ttk.Entry(file_row, textvariable=self.selected_file)
        input_entry.pack(side=LEFT, fill=X, expand=YES, padx=10)
        self._attach_tooltip(input_entry, self._t("Pfad der VCF-, gVCF-, 23andMe- oder FASTA-Eingabedatei"))
        browse_btn = ttk.Button(
            file_row,
            text=self._t("Durchsuchen"),
            command=self.choose_file,
            bootstyle="secondary-outline",
        )
        browse_btn.pack(side=LEFT)
        self._attach_tooltip(browse_btn, self._t("Eingabedatei auswählen"))
        ...
        af_entry = ttk.Entry(filter_box, textvariable=self.af_threshold, width=6)
        af_entry.pack(side=LEFT, padx=5)
        self._attach_tooltip(af_entry, self._t("Maximale Allelfrequenz vor dem Scan"))
        ...
        start_btn = ttk.Button(
            act_box,
            text=self._t("Pipeline starten"),
            command=self.on_start,
            bootstyle="success",
            width=18,
        )
        start_btn.pack(side=LEFT, padx=5)
        self._attach_tooltip(start_btn, self._t("Analyse der ausgewählten Datei starten"))
        stop_btn = ttk.Button(
            act_box,
            text=self._t("Stoppen"),
            command=self.on_stop,
            bootstyle="danger",
        )
        stop_btn.pack(side=LEFT, padx=5)
        self._attach_tooltip(stop_btn, self._t("Laufende Analyse stoppen"))
        ...
        cadd_entry = ttk.Entry(act_box, textvariable=self.cadd_highlight_threshold, width=4)
        cadd_entry.pack(side=LEFT, padx=2)
        self._attach_tooltip(cadd_entry, self._t("CADD-Hervorhebungsschwelle setzen"))
